...
$count = $db->get_row("Select count(*) as nr from ".DB_PREFIX."videos where pub > 0 and media=3 $where ");
$videos = $db->get_results("select * from ".DB_PREFIX."videos where pub > 0 and media = 3 $where $order ".this_limit()."");
//$db->debug();
?>
<div class="row-fluid">
						  ...
						  <?php foreach ($videos as $video) { ?>
                              <tr>
                                  <td><input type="checkbox" name="checkRow[]" value="<?php echo $video->id; ?>" class="styled" /></td>
                                   <td class="bord">
								<a class="tipS" title="<?php echo _lang("Unpublish"); ?>" href="<?php echo admin_url('images');?>&p=<?php echo this_page();?>&delete-video=<?php echo $video->id;?>"><i class="icon-trash"></i></a>
								  </td>
