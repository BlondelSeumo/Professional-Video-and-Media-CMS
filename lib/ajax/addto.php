...
if($check->nr > 0) {
echo json_encode(array("title"=>_lang('Already saved'),"text"=>_lang('This video was already in your playlist')));
} else {
$u = $db->get_row("SELECT owner FROM ".DB_PREFIX."playlists where id ='".$playlist."'");
if($u->owner <> user_id()) {
echo json_encode(array("title"=>_lang('Error'),"text"=>$u->owner._lang('You don\'t seem to own this playlist.')));

} else {
$db->query("INSERT INTO ".DB_PREFIX."playlist_data (`playlist`, `video_id`) VALUES ('".intval($playlist)."', '".$id."')");
echo json_encode(array("title"=>_lang('Added to playlist!'),"text"=>_lang('Video is now in your playlist.')));
}	
}
