<?php
	...
	*/
	...
	(
		...
		2 => 'Error establishing mySQL database connection.<br> <strong>Check the file vibe_config.php and server!</strong><br> Correct database user/password? <br> Correct hostname? <br> Database server running?',
	...
	*/
	...
	class ezSQL_mysql extends ezSQLcore
	{
		...
		*/
		...
		function connect($dbuser='', $dbpassword='', $dbhost='localhost')
		{
			...
			if ( ! $dbuser )
			{
				...
				$this->show_errors ? $this->logerrors($ezsql_mysql_str[1],E_USER_WARNING) : null;
			}
			...
			else if ( ! $this->dbh = @mysql_connect($dbhost,$dbuser,$dbpassword,true,131074) )
			{
				...
				$this->show_errors ? $this->logerrors($ezsql_mysql_str[2],E_USER_WARNING) : null;
			}
			else
			{
				$this->dbuser = $dbuser;
		...
		*/
		...
		function select($dbname='', $encoding='')
		{
			...
			if ( ! $dbname )
			{
				...
				$this->show_errors ? $this->logerrors($ezsql_mysql_str[3],E_USER_WARNING) : null;
			}
			...
			else if ( ! $this->dbh )
			{
				...
				$this->show_errors ? $this->logerrors($ezsql_mysql_str[4],E_USER_WARNING) : null;
			}
			...
			else if ( !@mysql_select_db($dbname,$this->dbh) )
			{
				...
				$this->show_errors ? $this->logerrors($str,E_USER_WARNING) : null;
			}
			else
			{
				$this->dbname = $dbname;
		...
		*/
		...
		/* PHPVibe error catching */
		function logerrors($error, $type=null) {
		$this->errors[] = $error;
		}		
